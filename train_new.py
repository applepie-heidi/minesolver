...
from brain_new import Brain


def main():
    choice = int(input("1. Train a new model from scratch \n2. Keep training a pre-trained model\n<1/2>? "))
    model_name = 'intermediate_model-conv2d1-37.py'  # input("What is the name of the model/model code? ")
    sessions = 10000  # int(input("How many learning sessions [10000]? "))
    samples = 200  # int(input("How many game moves (clicks) per session [200]? "))
